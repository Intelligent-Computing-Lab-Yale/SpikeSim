...
def compute_area(in_ch_list, in_dim_list, out_ch_list, out_dim_list, xbar_size, k, pe_per_tile, n_tiles, device):
    ...
    total_compute_ar_ov = Tile_ar_ov * n_tiles + np.sum(np.array(in_ch_list)*np.array(in_dim_list)*np.array(in_dim_list))* 0.7391*22
    ...
    digital_area = (Temp_Buff+Tile_buff+pe_per_tile*PE_ar)*n_tiles + np.sum(np.array(in_ch_list)*np.array(in_dim_list)*np.array(in_dim_list))* 0.7391*22
...
def latency_gen(in_ch_list, out_ch_list, out_dim_list, k, xbar, pe_per_tile, PE_cycle, time_steps):
    ...
    print(f' layerwise_latency {(np.array(halts)-np.array(starts))*PE_cycle*time_steps} ns')
